...
#### Scenario: Translate http server with authorization header to Cline
- **WHEN** universal server has `transport: "http"`, `url: "https://example.com/mcp"`, and `headers.Authorization: "Bearer API_KEY"`
- **THEN** output includes `"headers": { "Authorization": "Bearer API_KEY" }`
...
#### Scenario: Parse Cline remote server with authorization header
- **WHEN** reading `{ "url": "https://example.com/mcp", "type": "streamableHttp", "headers": { "Authorization": "Bearer API_KEY" }, "disabled": false }`
- **THEN** universal model has `transport: "http"`, `url: "https://example.com/mcp"`, `headers.Authorization: "Bearer API_KEY"`, `disabled: false`
